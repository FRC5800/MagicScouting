...
{#if scanning}
    ...
    <button onclick={stopScan} class="btn visible absolute bottom-[10%] w-[60%]">{$_('scanner.stop_button')}</button>
{/if}
    ...
    <Modal showModal={showModal} showX={false}>
        {#if stored == ''}
            ...
            <div class="box-border flex flex-row justify-between w-full">
                <button onclick={() => {HandleStore(() => {stored = 'success'; setTimeout(() => {stored = ''; showModal = false}, 1000)}, () => {stored = 'failure'; setTimeout(() => {stored = ''; showModal = false}, 1000)})}} class="w-[45%] p-2 shadow-md shadow-black bg-white active:shadow-inner text-black rounded-md">{$_('scanner.confirmation_modal.option_yes')}</button>
                <button onclick={() => {showModal = false; console.log(showModal)}} class="w-[45%] p-2 shadow-md shadow-black bg-white text-black rounded-md active:shadow-inner">{$_('scanner.confirmation_modal.option_no')}</button>
            </div>
