...
<button on:click={() => {showModal = false; goto("/settings"); isDataBaseSet = 'setting'}} class="mb-10 min-w-fit p-2 rounded-lg w-full btn mt-4 btn-primary hover:bg-primary-base bg-buttons border-buttons">{$_('home_page.database_alert.menu_button')}</button>